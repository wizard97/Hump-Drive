...
let state_for_dir dir_path =
  files_in_dir dir_path >>= fun filenames ->
  let filehashed' = List.map (fun fil -> hash_file (dir_path^Filename.dir_sep^fil)) filenames in

  let f = fun acc i -> i >>= fun e ->
    acc >>= fun lst -> Deferred.return (e::lst)
  in List.fold_left f (Deferred.return []) filehashed' >>= fun filehashes ->
  ...
  Deferred.return {dir_path = dir_path;
...
let changed_files dir_path acc (fname, modtime) =
  ...
  try
    ...
    if modtime <> stored_modtime then
      ...
      (FileMap.add fname (new_hash, modtime) file_map, StringSet.add fname queue) |> Deferred.return
    else (file_map, queue) |> Deferred.return
  with Not_found ->
     hash_file (dir_path^Filename.dir_sep^fname) >>= fun new_hash ->
